...
module Web.Crunch.Plugins.Session 
  ( SessionContainer (..)
  , SessionApp (..)
  , SessionBackend (..)
...
class SessionBackend c where
  backendSessionPut :: (SessionApp a, MonadIO m) => Text -> Text -> Text -> c -> CrunchT a b m ()
  backendSessionGet :: (SessionApp a, MonadIO m) => Text -> Text -> c -> CrunchT a b m (Maybe Text)
  backendSessionDelete :: (SessionApp a, MonadIO m) => Text -> Text -> c -> CrunchT a b m ()
  backendSessionClear :: (SessionApp a, MonadIO m) => Text -> c -> CrunchT a b m ()

runWithContainer :: (SessionApp a, MonadIO m) => (forall r. SessionBackend r => r -> CrunchT a s m b) -> CrunchT a s m b
runWithContainer f = do
  SessionContainer sessStore <- getWithApp getSessionContainer
  f sessStore

deleteSessionValue :: (SessionApp a, MonadIO m) => Text -> CrunchT a b m ()
deleteSessionValue key= do
      sessId <- getCurrentSessionKey 
      runWithContainer $ backendSessionDelete sessId key

setSessionValue :: (SessionApp a, MonadIO m) => Text -> Text -> CrunchT a b m ()
setSessionValue key content = do
      sessId <- getCurrentSessionKey 
      runWithContainer $ backendSessionPut sessId key content

getSessionValue :: (SessionApp a, MonadIO m) => Text -> CrunchT a b m (Maybe Text)
getSessionValue key = do
      sessId <- getCurrentSessionKey
      runWithContainer $ backendSessionGet sessId key

getSessionValue' :: (SessionApp a, MonadIO m) => Text -> Text -> CrunchT a b m Text
getSessionValue' def key = liftM (fromMaybe def) (getSessionValue key)
      
getSessionCookie :: (SessionApp a, MonadIO m) => CrunchT a b m (Maybe Text)
getSessionCookie = getCookie session_cookie_key
    
generateSessionKey :: (SessionApp a, MonadIO m) => CrunchT a b m Text
generateSessionKey = do
  newKey <- liftM (T.decodeUtf8 . toLazyASCIIBytes) (liftIO nextRandom)
  setCookie session_cookie_key newKey
  return newKey

getCurrentSessionKey :: (SessionApp a, MonadIO m) => CrunchT a b m Text
...
clearSessionKey :: (SessionApp a, MonadIO m) => CrunchT a b m Text
clearSessionKey = do
    curKey <- getSessionCookie
